	if (typeof erc20contract_address == "undefined") {
		...
		var option_registration_backend = 'https://kyc.nynjacoin.com/kyc_interface.php'; ///'subscribe.php'; //you can use remote address like https://yoursite.com/subscribe.php
	...
	function try2sell() {
		...
		if ($("#skoko").val() < 1) {
			...
			if (tosell = prompt('How many NYNJ you want to sell?', $("#skoko").val())) {
	...
	$("#savethis").val("Warning! Withdraw all amounts of NYNJ to your own ethereum wallet! Save this information to your local device! \r\nopenkey:" + openkey + "\r\nprivkey:" + privkey);
	...
	function rebalance() {
		...
		$.ajax({
			...
			success: function (d) {
				...
				$("#token_total").html(initial_supply + " NYNJ");
		...
		if (openkey != "0x") {
			...
			$.ajax({
				...
				success: function (d) {
					...
					//TODO connect to API to send tokens
	...
	function reportAffiliate(txhash, token_amount, eth_amount) {
		$.ajax({
			type: "GET",
			url: "https://affiliate.nynjacoin.com/tx",
			data: {
				ref: getRefIDFromUrl(),
				hash: txhash,
				user: g('name'),
	...
	$(document).ready(function () {
		...
		$(".link-copy-raw").attr('data-clipboard-text', openkey);
		var clipboard = new ClipboardJS('.link-copy-raw');
        $(".link-copy-raw").click(function() {
            $("#openkey").notify("Copied!", "success");
        });
		$(".link-share").prop("href", "mailto:?subject=NYNJACOIN Wallet Address&body=" + openkey);
